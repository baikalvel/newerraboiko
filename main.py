...
class Product(db.Model):  # создание базы данных
...
class Cart(db.Model):  # создание базы данных
    __tablename__ = "cart"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
...
class Order(db.Model):  # создание базы данных
    __tablename__ = "order"
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
...
@app.route('/', methods=['GET', 'POST'])  # открытие страници начальной
def index():
...
def register(): # Регистрация пользователя запись в бд
...
def login(): # авторизация ,проверка данных с бд
    if request.method == 'GET':
        ...
        global the_user
        the_user = email
        if User.query.filter_by(email=email, pw=pw, auth=1).first(): # в бд у админа в auth стоит 1 у обычного плебса 0
            global auth
            global logged_in
            auth = True
            logged_in = True
...
def logout():
    ...
    global the_user
    the_user = ""
    auth = False # значение прав админ
    logged_in = False # значение входа
    cart = Cart.query.all()
    for c in cart:
        c.quantity = 0
    db.session.commit()
    return redirect(url_for('index'))  # открытие страници


@app.route('/addproduct', methods=['POST', 'GET'])  # добавление продукта
def addproduct():
    if request.method == 'POST':
...
@app.route('/updateproduct', methods=['POST', 'GET'])  # обновление продукта
def updateproduct():
    if request.method == 'POST':
...
def deleteproduct():  # удаление продукта
    if request.method == 'POST':
...
def addtocart(productid, fromcart):  # добавление в корзину
    if request.method == 'POST':
        if fromcart == 'True':
...
def deletefromcart(productid): # удаление с корзины
    if request.method == 'POST':
        cart = Cart.query.filter_by(product_id=productid).first()
        cart.quantity -= 1
        db.session.commit()
...
def purchase():
    ...
    for c in cart:
        for p in products:
            if c.id == p.id:
                order = Order(user_id=user.id, product_id=p.id, datetime=now.strftime("%Y-%m-%d %H:%M"),
                              amount=c.quantity)
                p.stock -= c.quantity
